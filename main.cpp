...
int main(int argc, char *argv[]) 
{
  ...
  Double_t Temptot(0), ErrTemptot(0);
...
dir = opendir("./ForRemy/RunForAnalysis/Cold"); /// \brief open directory
      ...
      while ((openfile = readdir(dir)) != NULL) /// \brief loop on all files
	{
	  ...
	  if (name.compare(".") != 0 && name.compare("..") != 0)
	    { 
	      ...
	      cout<<"temperature = "<<temp<<" +/- "<<Errtemp<<endl;
	      Temptot=Temptot+temp;
	      ErrTemptot=ErrTemptot+Errtemp*Errtemp;
	      j2++;
	    }
	}
    }
  ...
  cout<<"Temptot="<<Temptot/j2<<" +/- "<<sqrt(ErrTemptot)/j2<<endl;
  ofstream filewarm;
  ofstream filecold;
  ofstream filew;
  ...
  if(HistoInputs)
      ...
      while(getline(HistoInputs, ligne))
	{
	  ...
	  Int_t k=i-detector*27; /// \brief 27 pour fe56 et 5 pour fe57
