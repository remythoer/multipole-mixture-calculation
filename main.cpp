...
dir = opendir("./ForRemy/RunForAnalysis/Cold"); /// \brief open directory
  ...
  // Histogram histo_cold("./ForRemy/RunForAnalysis/Cold_2015-04-22-62133");
  // Histogram warm("./ForRemy/RunForAnalysis/Warm_night_2015-04-21--62067");
  // Double_t temp, errTemp;
  // histo_cold.temp1(warm,1,temp,errTemp);
  // cout<<"T="<<temp<<"+/-"<<errTemp<<endl;
  ...
  // 	  Int_t Integrale = histo_cold.Integrate(detector,bin_g,bin_d-bin_g,bin_g-max_bg,bin_d+max_bg);
  // 	  Int_t k=i-detector*23;
  // 	  valeur_integrale_cold[k][detector]=valeur_integrale_cold[k][detector]+Integrale;
  ...
  // 	}
  //   }
  // else{cout<<"error file cold not open"<<endl;}
  // HistoInputs.close();
  ...
  for(Int_t i=0; i<peakref.size(); i++)
    {
      valeur_integrale[i][0]=valeur_integrale[i][0]/j1;
      valeur_integrale[i][1]=valeur_integrale[i][1]/j1;
      valeur_integrale[i][2]=valeur_integrale[i][2]/j1;
      valeur_integrale[i][3]=valeur_integrale[i][3]/j1;
  ...
  for(Int_t i=0; i<peakref.size(); i++)
    {
      valeur_integrale_cold[i][0]=valeur_integrale_cold[i][0]/j2;
      valeur_integrale_cold[i][1]=valeur_integrale_cold[i][1]/j2;
      valeur_integrale_cold[i][2]=valeur_integrale_cold[i][2]/j2;
      valeur_integrale_cold[i][3]=valeur_integrale_cold[i][3]/j2;
